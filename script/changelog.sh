...
change_log_header="# api-mini-tester (v$new_version) RELEASED\n"
change_log_changes=$(git log v"$old_version"..HEAD --oneline --no-merges --format='  * %s' | sort | uniq)
change_log_authors=$(git log v"$old_version"..HEAD --oneline --format='* %aN' | sort -u)
...
echo "Check updated CHANGELOG.MD: v$old_version -> v$new_version"
echo "Tag this version with v$new_version"
